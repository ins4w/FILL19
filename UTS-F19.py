...
while x==1:
        ...
        if os.path.exists(docu):
            ...
            with open(docu,'w') as archivo:
                for i in range(int(datos)):
                    ...
                    archivo.write(f"Nombres,Apellidos,Edad,Dirección,Teléfono,Sexo,Fecha de nacimiento,Recomendaciones\n{nombre},{apellido},{edad},{direccion},{telefono},{sexo},{nacimiento},{recom}\n")
        ...
        if os.path.exists(docu):
            with open(docu,'r') as archivo:
                if os.path.getsize(docu)==0:
                    ...
                    print(f"{Fore.GREEN}{archivo.read()}")
        #FALLIDA
        else:
        ...
        if os.path.exists(tuto):
            ...
            with open(tuto,'a') as tutorial:
                for i in range(int(datos)):
                    ...
                    recom=str(input("Recomendaciones: "))
                    print("---------------------------------------------"+Fore.GREEN+"\nSistema: Se han registrado los datos del paci\nente exitosamente."+Fore.RESET)
                    tutorial.write(f"{nombre},{apellido},{edad},{direccion},{telefono},{sexo},{nacimiento},{recom}\n")
        #FALLIDA
        else:
    ...
    #LIMPIEZA
        ...
        docu=name+".csv"
        #COMPROBACIÓN
        if os.path.exists(docu) and (os.path.getsize(docu)>=0):
            if os.path.getsize(docu)==0:
                    print(Fore.RED+f"Sistema: No se ha encontrado nada que limpiar\nen el archivo ({docu}).")
            else:
                with open(docu,'w') as archivo:
                    archivo.truncate(0)
                    print (Fore.GREEN+f"Sistema: El archivo ({docu}) ha sido limp\niado exitosamente.")
        #FALLIDA
        else:
            print(Fore.RED+f"Sistema: No se ha encontrado el archivo ({docu}).") 
    ...
    #BORRADO
        ...
        if os.path.exists(docu):
            os.remove(docu)
            print (Fore.GREEN+f"Sistema: El archivo ({docu}) ha sido elim\ninado exitosamente.")
        #FALLIDA
        else:
            print(Fore.RED+f"Sistema: No se ha encontrado el archivo ({docu}).")
    ...
    #SALIR
        ...
        print(Fore.GREEN+"Sistema: El programa ha finalizado exitosamen\nte.")
        print("---------------------------------------------")
        break